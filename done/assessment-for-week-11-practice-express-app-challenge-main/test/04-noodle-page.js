...
describe("The noodle page", () => {
  ...
  before(async () => {
    ...
    try {
      for (let i = 0; i < noodleOptions.length; i++) {
        ...
        if (new RegExp("Linguini").test(noodleOption.text())) {
          linguiniId = noodleOption.attr("value");
          break;
        }
      }
      if (!linguiniId) throw "No Linguini noodle found";
    } catch(e) {
      optionError = new Error(
        "Could not find a select dropdown with Linguini noodle to use to submit."
      );
      return;
    }
  ...
  describe("for some added pasta, Linguini noodle page only shows pastas with Linguini as their noodles", () => {
    it("the label", () => {
      if (!app) {
      ...
      for (let idx in pastas) {
        const { label, noodleId } = pastas[idx];
        if (linguiniId == noodleId) {
          const re = new RegExp(`<td[^>]*>\s*${label}\s*</td>`);
          expect(re.test(pageContent)).to.equal(
            true,
            `Could not find the label ${label} on the Linguini noodle page.`
          );
        } else {
          const re = new RegExp(`<td[^>]*>\s*${label}\s*</td>`);
          expect(re.test(pageContent)).to.not.equal(
            true,
            `Found the label ${label} on the Linguini noodle page.`
          );
        }
      }
    ...
    it("the description", () => {
      ...
      for (let idx in pastas) {
        const { label, description, noodleId } = pastas[idx];
        if (linguiniId == noodleId) {
          ...
          let namedRow = findNamedRow(label);
          expect(namedRow).to.equal('', `Found the label ${label} on the Linguini noodle page.`);
        }
      }
    });
    ...
    it("the taste", () => {
      ...
      for (let idx in pastas) {
        const { label, taste, noodleId } = pastas[idx];
        if (linguiniId == noodleId) {
          let namedRow = findNamedRow(label);
    ...
    it("the noodle and a link to the noodle's page", () => {
      ...
      for (let idx in pastas) {
        const { label, noodleId, noodle } = pastas[idx];
        if (linguiniId == noodleId) {
          let namedRow = findNamedRow(label);
    ...
    it("the sauce and a link to the sauce's page", () => {
      ...
      for (let idx in pastas) {
        const { label, sauceId, sauce, noodleId } = pastas[idx];
        if (linguiniId == noodleId) {
          let namedRow = findNamedRow(label);
          const $ = cheerio.load(namedRow);
