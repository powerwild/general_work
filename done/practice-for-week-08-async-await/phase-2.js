...
async function workout(totalTime) {
  try {
      const time = await stretch(totalTime);
      const time2 = await runOnTreadmill(time);
      const time3 = await liftWeights(time2);
      console.log(`done working out with ${time3 / 1000} seconds left`)
    } catch(err) {
