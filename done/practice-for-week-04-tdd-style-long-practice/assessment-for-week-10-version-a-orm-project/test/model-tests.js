...
describe('The model', () => {
  ...
  context('Queries', function () {
    it("can a fetch customer and associated Invoice data by customerId", async () => {
      const { lookupCustomerAndInvoicesById } = require('../queries/customerLookup.js');
      
      let record = await lookupCustomerAndInvoicesById(1);
      expect(record.name).to.eql('Riley Reeves');
      expect(record.Invoices.length).to.eql(1);

      record = await lookupCustomerAndInvoicesById(2);
      expect(record.name).to.eql('Jarrod Newman');
      expect(record.Invoices.length).to.eql(3);
    });

    it('can lookup a Customer by name', async () => {
      const { lookupCustomersByName } = require('../queries/customerLookup.js');

      let records = await lookupCustomersByName('Riley Reeves');
      expect(records[0].id).to.eql(1);
      expect(records[0].Invoices).to.equal(undefined); // not eagerly loaded

      records = await lookupCustomersByName('Jarrod Newman');
      expect(records[0].id).to.eql(2);
      expect(records[0].Invoices).to.equal(undefined); // not eagerly loaded
    });
    ...
    it('can lookup a Customer by Invoice Id', async () => {
      ...
      let record = await lookupCustomerByInvoiceNumber('E0M 3P9');
      expect(record.name).to.equal('Riley Reeves');

      record = await lookupCustomerByInvoiceNumber('Z1N 0Y0');
      expect(record.name).to.equal('Jarrod Newman');
    });
